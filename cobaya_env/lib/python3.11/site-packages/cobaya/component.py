...
def load_external_module(
    ...
    print(f"[DEBUG-RGH] load_external_module() called with module_name={module_name}, path={path}, install_path={install_path}")
    ...
    try:
        if is_main_process():
            ...
            print(f"[DEBUG-RGH] _bare_load_external_module() about to be called with load_kwargs={load_kwargs}")
        module = _bare_load_external_module(not_installed_level="debug", **load_kwargs)
    except ComponentNotInstalledError:
        print(f"[DEBUG-RGH] FAILED initial load for {module_name} at path={path}")
        if default_global:
            if is_main_process():
    ...
    print(f"[DEBUG-RGH] SUCCESS loading module {module_name} from {module.__file__}")
    return module
